---
title: "Aqueous Solubility Prediction: QSPR vs Graph Neural Networks"
description: "A comparative study of aqueous solubility prediction with QSPR vs GCN"
date: "May 31 2025"
repoURL: "https://github.com/rnepal2/Solubility-Prediction-with-Graph-Neural-Networks"
---
...
In this project, we are testing two different classes of prediction models to classify compounds as either "soluble" or "poorly soluble." We'll explore two computational tools for this:
...
Any machine learning adventure starts with good data. For this test, we used a dataset of about 9,000 small organic molecules, originally published in a [Scientific Data article](https://www.nature.com/articles/s41597-019-0151-1). Each molecule in this collection comes with an experimentally measured aqueous solubility value, noted as **logS** (that's the logarithm of its molar solubility in mol/L).
...
Alright, moment of truth! We pitted our QSPR LightGBM model against our GCN to see which one was better at classifying compounds as "soluble" or "insoluble." We used a test set of molecules the models had never encountered.
...
**A Quick Take on Strengths & Weaknesses:**
...
*   **Graph Convolutional Network (GCN):**
    ...
    *   *But,* in our case, it didn't quite top the QSPR. GCNs often need lots of data, can be trickier to tune, and are sometimes seen as "black boxes."

## Conclusion and Future Outlook

In our face-off between a QSPR (LightGBM) model and a GCN for classifying aqueous solubility, the QSPR method came out slightly on top based on key metrics like AUC and accuracy. It's a great reminder that while newer methods like GCNs are exciting and powerful, traditional approaches with well-chosen features can still be highly competitive.

This project is just one example test of what is possible. There are many exciting ways to push this further:

*   **Beyond Yes/No (Regression):** Instead of just classifying, we could train models to predict the actual logS value.
*   **Trying New Model Flavors:** We could experiment with different types of GNNs (like Graph Attention Networks) or explore other QSPR algorithms.
*   **Bigger, More Diverse Data:** Training on larger and more varied chemical data could make models more robust.
*   **Peeking Inside the "Black Box" (Interpretability):** Digging into techniques like attention mechanisms can help us understand GNN decision-making.
*   **Best of Both Worlds (Hybrid Models):** Combining QSPR with GNNs is a promising area.
*   **Predictions with Confidence (Conformal Prediction):** Adding statistically sound confidence levels to predictions.
